...
/*
  Tuple Tag (64 bits)
  #b|- 7 bit unused -|- 50 bit field [50, 0] -| 6 bits length -| 1 bit
  isNotForwarding Pointer
...
int any_tag(int64_t any) { return any & ANY_TAG_MASK; }
...
void collect(int64_t **rootstack_ptr, uint64_t bytes_requested) {
  ...
  if (sizeof(int64_t) * (fromspace_end - free_ptr) < bytes_requested) {
    ...
    */
    ...
    unsigned long occupied_bytes =
        (free_ptr - fromspace_begin) * sizeof(int64_t);
...
void cheney(int64_t **rootstack_ptr) {
  ...
  for (int64_t **root_loc = rootstack_begin; root_loc != rootstack_ptr;
       ++root_loc) {
    /*
...
void print_int(int64_t x) { printf("%" PRId64, x); }
...
void print_vecbegin() { printf("#("); }

void print_space() { printf(" "); }

void print_vecend() { printf(")"); }

void print_ellipsis() { printf("#(...)"); }
